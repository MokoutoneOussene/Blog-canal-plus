            ...
            <div class="row">
                <div class="col-md-9 post-content" data-aos="fade-up">
                    ...
                    <div class="single-post">
                        <div class="post-meta"><span class="date">{{ $finds->lieu }}</span> <span
                                class="mx-1">&bullet;</span> <span>{{ $finds->date }}</span></div>
                        <h1 class="mb-5">{{ $finds->nom_pub }}</h1>
                        <p>{{ $finds->descript }}</p>
                    ...
                    <div class="row justify-content-center mt-5">
                        <div class="col-lg-12">
                            ...
                            <form method="POST" action="{{ route('gestion_comment.store') }}">
                                @csrf
                                <div class="row">
                                    <input name="users_id" type="text" value="{{ Auth::User()->id }}" hidden>
                                    <input name="publications_id" type="text" value="{{ $finds->id }}" hidden>
                    ...
                    <div class="aside-block">
                        ...
                        <ul class="aside-links list-unstyled">
                            @foreach (App\Models\Categorie::all() as $cate)
                                <li>
                                    <a href="{{ url('categorie_publication/' . $cate->id) }}"><i class="bi bi-chevron-right"></i> {{ $cate->nom_cat }}</a>
                                </li>
                            @endforeach
                        </ul>
                    </div><!-- End Categories -->
